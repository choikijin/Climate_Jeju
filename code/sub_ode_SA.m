function [f] = sub_ode_SA_0405(parameter,xdata)

global dt init_C end_time K num_change_param
global mu1 mu2 mu3 mu4 mu5 rho temp_inter a develop_rate1 develop_rate2 hum_inter
...
temp_inter = temp_inter*c_temp;
hum_inter = hum_inter*c_hum;
...
d1_sum(1)=dt*sum(d1(1:1/dt),2); d2_sum(1)=dt*sum(d2(1:1/dt),2); d3_sum(1)=dt*sum(d3(1:1/dt),2); 

X1(1) = init_C(1); X2(1) = init_C(2); X3(1) = init_C(3); 
X4(1) = init_C(4); X5(1) = init_C(5); Total(1) = init_C(2)+init_C(3)+init_C(4)+init_C(5); 

for fi = 1:sz
    X1(fi+1) = X1(fi) + (d4(fi)*rho*X4(fi) - (d1(fi)+mu1)*X1(fi))*dt;
    X2(fi+1) = X2(fi) + (d1(fi)*(1-Total(fi)/K)*X1(fi) - (d2(fi)+mu2)*X2(fi))*dt;
    X3(fi+1) = X3(fi) + (d2(fi)*X2(fi) - (d3(fi)+mu3)*X3(fi))*dt;
    X4(fi+1) = X4(fi) + (a*d3(fi)*X3(fi) - (d4(fi)+mu4)*X4(fi))*dt;
    X5(fi+1) = X5(fi) + (a*d3(fi)*X3(fi) - mu5*X5(fi))*dt;
...
Totalsum(1) = dt*sum(Total(1:1/dt),2);
...
f=[Final_inci_Lar', Final_inci_Lym', Final_inci_Adu'];
end
