function [f] = sub_ode_tick_simple_control_monthly_ThM(parameter, xdata, month_list, control_list)
...
if double([strcmp(control_list,["Mowing" ""])]) == [1 1]
    ...
    for fi = 1:sz
        ...
        if mod(fix(fi*dt)-m_list(1),12) == 0 || mod(fix(fi*dt)-m_list(2),12) == 0 || mod(fix(fi*dt)-m_list(3),12) == 0
    ...
    for fj = 1:mn-1
        if mod(fj-m_list(1),12) == 0 || mod(fj-m_list(2),12) == 0 || mod(fj-m_list(3),12) == 0
            idx_dev = (fj-1)/dt+2;
